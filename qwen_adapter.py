...
class QwenAdapter(BaseAdapter):
    ...
    def __init__(self, api_key: str, base_url: str = "https://dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation"):
    ...
    async def identify_ingredients(self, image_bytes: bytes) -> List[Ingredient]:
        ...
        try:
            ...
            data = response.json()
            ingredients_data = json.loads(data['output']['text'])
    ...
    async def analyze_nutrition(self, week_plan: WeekPlan) -> NutritionReport:
        ...
        try:
            ...
            response.raise_for_status()
            data = response.json()
            nutrition_data = json.loads(data['output']['text'])
    ...
    async def generate_shopping_list(self, recipes: Dict[str, Any], have: List[Ingredient]) -> ShoppingList:
        ...
        try:
            ...
            response.raise_for_status()
            data = response.json()
            shopping_list_data = json.loads(data['output']['text'])
            return ShoppingList(items=shopping_list_data)
        except Exception as e:
            print(f"Qwen-VL 购物清单生成错误: {e}")
